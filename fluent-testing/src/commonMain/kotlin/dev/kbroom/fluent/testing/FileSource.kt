...
data class FileSource(
    ...
    fun getPathForLocale(locale: String): String {
...
fun String.toResourceId(type: ResourceType = ResourceType.Required): ResourceIdWithType =
    ResourceIdWithType(this, type)

class TestEnvironment(
    private val basePath: String,
    private val fileSources: List<FileSource>
) {
    ...
    ): Map<LanguageIdentifier, dev.kbroom.fluent.bundle.FluentBundle> {
        return resourceManager.getBundles(locales, resourceIds.map { ResourceId(it.id) })
    }
    
    fun getBundle(
        locales: List<LanguageIdentifier>,
        resourceIds: List<ResourceIdWithType>
    ): dev.kbroom.fluent.bundle.FluentBundle {
        return resourceManager.getBundle(locales, resourceIds.map { ResourceId(it.id) })
    }
}

fun fileSource(name: String, path: String, locales: List<String>) =
    FileSource(name, path, locales)
